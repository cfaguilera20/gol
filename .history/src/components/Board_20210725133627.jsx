                ...
                {world.board.map((row, rowIdx) => (
                    <div key={rowIdx} className="row">
                        {row.map((isAlive, cellIdx) => {
                            const className = isAlive ? 'cell cell-green' : 'cell';
                            const neighborsAlive =
                                world.getNeighborsAliveCounter(x, y);
                            return (
...
class World {
    ...
    createBoard() {
        for (let y = 0; y < this.size; y++) {
            ...
            for (let x = 0; x < this.size; x++) {
                const isAlive = Math.random() > 0.9 ? true : true;
    ...
    getNeighborsAlive(x, y) {
        return this.getNeighbors(x, y).filter((isAlive) => isAlive === true);
    }
